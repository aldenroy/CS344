...
int main(){
    ...
    //Set up signal masks
    //Source from exploration on API signal handling and lecture
    ...
    //Samantha on the discord brought up SIG_IGN and some references to this stuff		

    //added this cause the test execution on canvas looked like it needed it at the start.
    printf("$ smallsh\n");
    fflush(stdout);
    char* args_line[2048]; //to get args from command line.
    int num_args;
    ...
    while(token != NULL){//now I need to parse and add commands into array
        ...
        for(int i = 0; i < strlen(args[num_args]); i++){
            ...
            if(args[num_args][i] == '$' && args[num_args][i - 1] == '$'){
				...
				snprintf(temp, 2048, "%s%d", args[num_args], getpid());
    ...
    switch (pid){
        ...
        case 0://child process section
            ...
            for(int i = 0; args[i] != NULL; i++){
                ...
                //this is the section for the input redirection which if received will change the bool to received and copy args to input
                ...
                //this is the section for the output redirection which is essentially the same, just for the output
            }
            ...
            if(input_received){
                ...
                //create duplicate of file descriptor https://linux.die.net/man/2/dup2
                close(input_descriptor);
            }

            if(output_received){
                int output_descriptor = 0;
                //okay we do need error handling for this section so this will cause error if file can't open.
...
void run_status(int *error_num) {
    ...
    //I believe samantha on the discord also brought up this stuff
    if(error + signal == 0){
        val = 0;
    }
    else{
...
void exit_shell(){
    ...
    //go through and kill all processes like canvas says to do
    else{
